...
def active_scan(){
    ...
    sites.sites.each{ e ->
         println(e)
         def scan = sh(returnStdout: true, script: " curl \"http://zap:8090/JSON/ascan/action/scan/?apikey=change-me-9203935709&url="+e+" &recurse=true&inScopeOnly=&scanPolicyName=&method=&postData=&contextId=\"").trim();
         ...
         while(status != 100){
             ...
             def getStatusJson = readJSON text: getStatus;
             status = getStatusJson.status.toInteger();
         }
    } 
